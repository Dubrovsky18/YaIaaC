resource "yandex_compute_instance" "deployment" {
  ...
  boot_disk {
    disk_id = yandex_compute_disk.ssd_deployment.id
  }
  ...
  #     "sudo apt-get install salt-deployment salt-ssh salt-syndic salt-cloud salt-api",
  #     "sudo systemctl enable salt-deployment --now",
  #     "sudo systemctl enable salt-syndic --now",
  #     "sudo systemctl enable salt-api --now",
  #     "echo 'interface: ${self.network_interface.0.ip_address}' > /etc/salt/deployment.d/network.conf",
  #     "echo 'ret_port: 4506' >> /etc/salt/deployment.d/network.conf",
  #     "echo 'publish_port: 4505' >> /etc/salt/deployment.d/network.conf",
  #     "sudo salt-key",
  #     "sudo systemctl restart salt-deployment"
  #     ]
  #     }
}


resource "yandex_compute_disk" "ssd_deployment" {
  type = "network-ssd"
  zone = "ru-central1-a"
  size = 20
...
output "ip-deployment-public" {
    value = yandex_compute_instance.deployment.network_interface.0.nat_ip_address
}

output "ip-deployment-privat" {
    value = yandex_compute_instance.deployment.network_interface.0.ip_address
}
